...
class Proposals_model extends CRM_Model
{
    ...
    public function get($id = '', $where = [], $for_editor = false)
    {
        ...
        //     $sql = "SELECT tblproposals.*  ,tblcurrencies.id as currencyid, tblproposals.id as id, tblcurrencies.name as currency_name FROM tblproposals LEFT JOIN tblcurrencies ON  
        // tblcurrencies.id = tblproposals.currency WHERE tblproposals.id = ? ";
        ...
        // $this->db->query($sql, [$id]);
            ...
            //SELECT *FROM tblproposals LEFT JOIN tblclient ON tblproposals.client_id = tblclient.userid;
            $querystuff = $this->db->query("SELECT *FROM tblproposals LEFT JOIN tblstaff ON tblproposals.addedfrom = tblstaff.staffid WHERE tblproposals.id='$id'  ");
            $return['stuffdetails'] = $querystuff->result_array();
            $query = $this->db->query("SELECT *FROM tblproposals LEFT JOIN tblclients ON tblproposals.client_id = tblclients.userid WHERE tblproposals.id='$id'  ");
            $return['tblcustomfieldsvalues'] = $query->result_array();
    ...
    public function get_relation_data_values($rel_id, $rel_type)
    {
        ...
        if ($rel_type == 'customer') {
            ...
            $data->titles    = $_data->titles;
            $data->client_name    = $_data->client_name;
            $data->surname    = $_data->surname;
            $data->dob    = $_data->dob;
            $data->gender    = $_data->gender;
            $data->marital_status    = $_data->marital_status;
            $data->mail_address    = $_data->mail_address;
            $data->cell_number    = $_data->cell_number;
            $data->physical_address    = $_data->physical_address;
            $data->beneficiary_surname    = $_data->beneficiary_surname;
            $data->beneficiary_contacts    = $_data->beneficiary_contacts;
            $data->beneficiary_id_number    = $_data->beneficiary_id_number;
            $data->beneficiary_bank_acc    = $_data->beneficiary_bank_acc;
            $data->relationship    = $_data->relationship;
            $data->identity_type    = $_data->identity_type;
            $data->retirement_age    = $_data->retirement_age;
            $data->identity_number    = $_data->identity_number;
            $data->country_id_issued    = $_data->country_id_issued;
            $data->academic_qualification    = $_data->academic_qualification;
            $data->clientbank_name    = $_data->clientbank_name;
            $data->clientbank_acc    = $_data->clientbank_acc;
            $data->client_bank_branchcode    = $_data->client_bank_branchcode;
            $data->beneficiary_email    = $_data->beneficiary_email;
            $data->contact_number    = $_data->contact_number;
            $data->beneficiary_name    = $_data->beneficiary_name;
            

            function get_customed_data($rel_id) {

                $this->db->select( '*' ); 
                $this->db->from( 'custom_relationships' );
                $this->db->join( 'tblclients', 'tblclients.id = custom_relationships.client_id' , 'left' );
                $this->db->join( 'tblcustomfields', 'tblcustomfields.id = custom_relationships.field_id' , 'left' );
                $this->db->where( 'custom_relationships.field_id', $rel_id);
                $query = $this->db->get();
                return $query->result();
             }
