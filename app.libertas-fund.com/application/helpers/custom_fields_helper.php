...
function render_custom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [])
{
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            $fielded = $CI->db->get('tblcustomfieldsvalues')->result_array();
            // var_dump(serialize($fielded[0]));
            $field_name = $field['name'];
        //     foreach ($fielded as $key => $valued) {
        //         var_dump($key['value']);
        //        if($valued['relid'] == $field['id']) {
        //         $valueb = $valued['value'];
        ...
        $custom_fields = get_custom_fields('customers');
            foreach ($custom_fields as $field) {
                $valued = get_custom_field_value($rel_id, $field['id'], 'customers', false);
                if ($valued == '') {
                    continue;
                }
                return $valued;
...
function render_clientcustom_fields($belongs_to, $rel_id = false, $where = [], $items_cf_params = [])
{
    // Is custom fields for items and in add/edit
    $items_add_edit_preview = isset($items_cf_params['add_edit_preview']) && $items_cf_params['add_edit_preview'] ? true : false;
    ...
    $CI->db->order_by('field_order', 'asc');
    $fields = $CI->db->get('tblcustomfields')->result_array();
    $fieldsvalue = $CI->db->get('tblcustomfieldsvalues')->result_array();
    $fields_html = '';
    ...
    if (count($fields)) {
        ...
        foreach ($fields as $field) {
            ...
            $fielded = $CI->db->select('custom_relationships.field_id, custom_relationships.value, custom_relationships.client_id, tblcustomfields.name');
            $fielded = $CI->db->join('tblcustomfields', 'tblcustomfields.id = custom_relationships.field_id', 'left');
            //$fielded = $CI->db->group_by('client_id');
            $fielded = $CI->db->get('custom_relationships')->result_array();
            
            foreach($fielded as $key=>$item)
            {
               
                if(similarity($field['id'], $item['field_id']))
                    {
                        $field_name = $field['name'];
                        // var_dump($item);
                        $value = $item['value'];
